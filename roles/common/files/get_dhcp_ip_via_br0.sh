...
if [ "$#" -ne "1" ]; then
        echo "tl;dr: modified pipework script to get a (dhcp) ip via br0"
...
case "$N" in
  0)
    ...
    if installed docker; then
      ...
      [ "$DOCKERPID" = 0 ] && {
        die 1 "Docker inspect returned invalid PID 0 for $GUESTNAME"
      }
...
case "$DHCP_CLIENT" in
  dhcp)
    docker run --net container:$GUESTNAME --cap-add NET_ADMIN \
